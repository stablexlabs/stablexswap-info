...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(STABLEXSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(STABLEXSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })

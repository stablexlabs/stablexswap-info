...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name stablexswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name stablexswap return';
  }
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  STABLEXSWAP_RETURN: 'STABLEXSWAP_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.STABLEXSWAP_RETURN) {
          return p0?.stablexswap?.return > p1?.stablexswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.STABLEXSWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.STABLEXSWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.STABLEXSWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
